...
import * as PopupMenu from 'resource:///org/gnome/shell/ui/popupMenu.js'; // Corrected import
...
export default class CollapsibleAppTrayExtension extends Extension.Extension {
    ...
    _createTrayButton() {
        ...
        Util.Logger.debug(`Tray menu created, initial children: ${this._trayBox.get_children().length}`);
    ...
    _setupAppTracking() {
        ...
        this._appRunningHandler = this._appSystem.connect('app-state-changed', 
            (appSystem, app) => this._onAppStateChanged(app));
    ...
    _populateTrayIcons() {
        ...
        Object.values(Main.panel.statusArea).forEach(icon => {
            if (icon instanceof IndicatorStatusIcon.IndicatorStatusIcon ||
    ...
    _addTrayIcon(icon) {
        ...
        menuItem.connect('button-press-event', (actor, event) => {
            if (event.get_button() === Clutter.BUTTON_PRIMARY) {
                if (icon.menu) {
                    Util.Logger.debug(`Opening submenu for: ${id}, reactive: ${icon.menu.actor.reactive}, focused: ${icon.menu.actor.has_key_focus()}`);
                    
                    // Get absolute screen position of the menu item
                    const [itemX, itemY] = menuItem.get_transformed_position();
                    const [itemWidth, itemHeight] = menuItem.get_transformed_size();
                    const screenWidth = Main.uiGroup.get_width();
                    const screenHeight = Main.uiGroup.get_height();

                    // Dynamic positioning based on icon location
                    let x, y;
                    // Horizontal: Open to the right if space, else left
                    if (itemX + itemWidth + icon.menu.width < screenWidth) {
                        x = itemX + itemWidth; // Right of icon
                    } else {
                        x = itemX - icon.menu.width; // Left of icon
                    }
                    // Vertical: Open below if space, else above
                    if (itemY + itemHeight + icon.menu.height < screenHeight) {
                        y = itemY + itemHeight; // Below icon
                    } else {
                        y = itemY - icon.menu.height; // Above icon
                    }

                    // Apply positioning and open submenu
                    icon.menu.open(BoxPointer.PopupAnimation.SLIDE, x, y);
                    icon.menu.actor.grab_key_focus();
                    Util.Logger.debug(`Submenu opened for: ${id} at (${x}, ${y}), reactive: ${icon.menu.actor.reactive}, focused: ${icon.menu.actor.has_key_focus()}`);
    ...
    _removeTrayIcon(icon) {
        const id = icon.uniqueId || icon.title || 'unknown-tray-icon';
        const menuItem = this._trayIcons.get(id);
        if (menuItem) {
            Util.Logger.debug(`Removing tray icon: ${id}`);
            this._trayBox.remove_child(menuItem);
            menuItem.destroy();
            this._trayIcons.delete(id);
        }
