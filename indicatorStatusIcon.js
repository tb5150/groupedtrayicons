...
class IndicatorStatusIcon extends BaseStatusIcon {
    _init(indicator) {
        ...
        // Ensure submenu is managed by global menu manager
        if (this.menu) {
    ...
    _updateMenu() {
        ...
        if (this.visible && this._indicator.menuPath) {
            ...
            if (this._menuClient.isReady) {
                ...
                Util.Logger.debug(`Menu client not ready for ${this.uniqueId}, using fallback`);
    ...
    vfunc_button_press_event(event) {
        ...
        if (event.get_button() === Clutter.BUTTON_SECONDARY || 
            ...
            if (this.menu.numMenuItems) {
                Util.Logger.debug(`Toggling menu for ${this.uniqueId}`);
                this.menu.toggle();
